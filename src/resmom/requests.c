...
req_cred(struct batch_request *preq) /* ptr to the decoded request   */
{
	...
	if (decode_block_base64((unsigned char *)preq->rq_ind.rq_cred.rq_data, preq->rq_ind.rq_cred.rq_size, out_data, &out_len, msg, msg_len) != 0) {
	...
	store_or_update_cred(preq->rq_ind.rq_cred.rq_jobid, preq->rq_ind.rq_cred.rq_princ, data, data_base64);

	/* if we have a running job renew ticket */
