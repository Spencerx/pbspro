 ...
 *	not too old. Such a credentials are returned. If they are not present
 ...
 */
static
struct cred_cache *get_cached_cred(job  *pjob)
{
	...
	while (fgets(buf, BUFSIZE, fp) != NULL) {
		...
		/* last line in buf is credential in base64 */
	}
	if((ret = pclose(fp)))  {
	...
	if ((cred->data = (char *)malloc(cred->size * sizeof(char))) == NULL) {
		log_err(errno, __func__, "Unable to allocate Memory!\n");
		free(cred);
		return NULL;
	}
...
setup_cred(struct batch_request *preq, job  *pjob)
{
	...
	if ((preq->rq_ind.rq_cred.rq_data = (char *)malloc(cred->size * sizeof(char))) == NULL) {
		log_err(errno, __func__, "Unable to allocate Memory!\n");
